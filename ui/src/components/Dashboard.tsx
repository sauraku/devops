            ...
            <table className="w-full text-left border-collapse min-w-[650px] border-b border-line">
              <thead>
                <tr className="border-b border-line text-[9px] text-muted uppercase tracking-widest font-bold bg-surface-3/30">
                  ...
                  <th className="py-2.5 px-4 w-28 font-mono">Port</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-line font-mono text-xs">
                {containerList.length > 0 ? (
                  containerList.map(([service, containerState]) => {
                    ...
                    const getStatusClass = (stateStr: string) => {
                      if (stateStr === 'running') return 'bg-good text-accent-on font-bold';
                      if (stateStr === 'unavailable') return 'bg-muted text-accent-on font-bold';
                      return 'bg-warn text-accent-on font-bold';
                    };

                    const getHealthClass = (hlth: string) => {
                      if (hlth === 'healthy') return 'bg-good text-accent-on font-bold';
                      if (hlth === 'unhealthy') return 'bg-bad text-accent-on font-bold';
                    ...
                    return (
                      <tr key={service} className="hover:bg-surface-2/20 transition-colors group">
                        ...
                        <td className="p-0 border-r border-line h-11 w-36">
                          <div className={`w-full h-full flex items-center justify-center text-[10px] uppercase tracking-wider select-none ${getStatusClass(containerState)}`}>
                            {containerState}
                          </div>
                        </td>
                        <td className="p-0 border-r border-line h-11 w-36">
                          <div className={`w-full h-full flex items-center justify-center text-[10px] uppercase tracking-wider select-none ${getHealthClass(health?.status ?? 'unknown')}`}>
                            {health?.status ?? 'unknown'}
                          </div>
                        </td>
                        <td className="py-3 px-4 text-muted text-[11px]">
                          {health?.detail?.match(/\d+->\d+/) ? health.detail.match(/\d+->\d+/)?.[0] : 'dynamic'}
                        </td>
                      </tr>
                    );
                  })
